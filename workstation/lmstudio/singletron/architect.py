...
def fill_module_workflow_with_llm(host: str, port: int, module_name: str, source_code: str, functions_info: str, classes_info: str, debug: bool = False) -> str:
    ...
    # Updated Prompt: Asking for a comprehensive explanation instead of 1-2 sentences.
    system_prompt = (
    ...
    payload = {
        ...
        "temperature": 0.2, # Slightly bumped up to allow for a more natural paragraph
        "max_tokens": 8192
    }
...
def update_placeholders(ast_data: dict, name: str, description: str, class_name: str = None, is_workflow: bool = False):
    ...
    if not class_name:
        ...
        # Update root-level class descriptions (Replacing the placeholder maintains multiline formatting)
...
def main():
    ...
    parser.add_argument('--output', type=str, default='archer.yaml', help='Path to save the consolidated architectural report')
    ...
    try:
        ...
        print(f"[!] Failed to parse YAML syntax. Ensure the input file is valid YAML. Details:\n{e}")
        return
    ...
    if 'Module' in ast_data and isinstance(ast_data['Module'], dict):
        ...
        # Enriched Context: Now extracting args and returns to send to the LLM alongside the descriptions
        func_summaries = []
        for f in funcs_list:
            for k, v in f.items():
                sig = ""
                # Safely pull the original args and returns from the FunctionDef block if they exist
    ...
    ensure_multiline_bodies(ast_data)

    # Save the consolidated YAML
    with open(args.output, 'w', encoding='utf-8') as f:
        yaml.safe_dump(ast_data, f, default_flow_style=False, sort_keys=False, width=float("inf"))
